...
/**      
 *  Jednoduchý databázový wrapper
 *  @author Chlumecký Ondřej <ondrej.chlumecky@malfini.com>
 */
class Db
{
    /**
     * @ignore
     */
    const loginDB = array("local" => array("host" => "localhost","database"=>"login","user"=>"root","password"=>"admin"));
	 ...
	 */
	public static function connect($nameDB)
	{
		try{
            $host = self::loginDB[$nameDB]['host'];
            $database = self::loginDB[$nameDB]['database'];
            $user = self::loginDB[$nameDB]['user'];
            $password = self::loginDB[$nameDB]['password'];
        ...
        catch (PDOException $e)
        {
            echo ('<!DOCTYPE html>
                    ...
                    <h3 align="center">500 Internal server error</h3>
                    <div align="center"><img src="assets/img_search/Error-500-alert.gif"></div>
